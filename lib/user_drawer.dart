...
class _UserDrawerState extends State<UserDrawer> {
  ...
  Future<void> _switchToProvider() async {
    ...
    // Navigator capture karo BEFORE async gap
    final navigator = Navigator.of(context);
    ...
    try {
      ...
      // Firestore se check karo agar already provider hai
      ...
      if (providerDoc.exists) {
        ...
        // Setup complete hai? ProviderHomeScreen
        if (pData['setupStep'] == 'complete' && pData['isVerified'] == true) {
          navigator.push(
            MaterialPageRoute(builder: (_) => const ProviderHomeScreen()),
          );
        } else {
          // Setup adha tha — wahan se continue karo
          navigator.push(
            MaterialPageRoute(
        ...
        // Naya provider — setup flow start
        navigator.push(
